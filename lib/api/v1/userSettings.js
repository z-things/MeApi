                ...
                async.waterfall([
                    ...
                    function (error, user) {
                        if (error) {
                            ...
                            logger.debug(user.extra.settings);
                            var found = false;
                            if (util.isNullOrUndefined(user.extra.settings)) {
                                user.extra.settings = [];
                            }
                            for (var i = 0, len = user.extra.settings; i < len; ++i) {
                                if (user.extra.settings[i].name === parameter.name) {
                                    user.extra.settings[i].value = parameter.value;
                                    found = true;
                                    break;
                                }
                            }
                            if (!found) {
                                user.extra.settings.push(parameter);
                            }
                            var cmd = {

...
var filling = function (request, parameter) {
  var phoneNumber = parameter.extra.phoneNumber;
  var countryCode = phoneNumber.substring(phoneNumber.indexOf("+"), phoneNumber.indexOf(")"));
  if (util.isNullOrUndefined(parameter.name) || parameter.name === "") {
    var pos = phoneNumber.indexOf(")");
    parameter.name = phoneNumber.substr(pos + 1);
  }
  if (util.isNullOrUndefined(parameter.extra.companyName)) {
    parameter.extra.companyName = "weiwiz";
  }
  if (util.isNullOrUndefined(parameter.extra.timeZone)) {
    parameter.extra.timeZone = "0";
  }
  /*if (util.isNullOrUndefined(parameter.extra.language)) {
      request.body.extra.language = "en";
  }*/
  if (util.isNullOrUndefined(parameter.type)) {
    request.body.type = {
      id: "060A08000000",
      name: "user",
      icon: ""
  ...
  if (util.isNullOrUndefined(parameter.extra.email)) {
    ...
    utilCommon.getUser(conx, userId,
      function (user) {
        if (user !== null) {
          ...
          res.weiwiz.MHome.response = user;
        }
        else {

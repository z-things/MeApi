        ...
        utilCommon.sendMsg(conx, conx.configurator.getConfRandom("services.timer"), cmd, function (response) {
            ...
            if (response.retCode === 200) {
                if (timerId === "*") {
                    ...
                    if (util.isNullOrUndefined(res.response.automatics)) {
                        res.weiwiz.MHome.response.automatics = []
                    }
                    else {
                        //解析得到定时器概要信息
                        parse2Abstract(response.data, getUserLanguage(conx, userId), function (result) {
                            res.weiwiz.MHome.response.automatics = result;
                        });
                    }
                }
                else {

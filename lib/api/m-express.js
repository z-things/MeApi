...
var resourceError = [203004, 208001, 209001, 200006];
...
function MExpress(conx, uuid, token, configurator) {
    ...
    this.listen = function () {
        ...
        app.get('/m-home/rest/v1/system/settings/user/forgot', function (request, response) {
            var proof =
                {
                    phoneNumber: request.query["proof"]
                };
        ...
        app.post('/m-home/rest/v1/users/me/settings', function (request, response) {
            userSettingsV1Case.post(request, self.getUserId(request), self.getUserToken(request), function (result) {
                self.sendMsg(response, result);
            });
        });
        app.get('/m-home/rest/v1/users/me/settings', function (request, response) {
        ...
        app.get('/m-home/rest/v1/system/settings/wrappers/:markid/helps', function (request, response) {
            systemSettingsV1Case.getHelp(request, request.params.markid, response);
        });
        //todo
        app.get('/m-home/rest/v1/system/settings/wrappers/:markid/versions/:versionId', function (request, response) {
            logger.debug("/m-home/rest/v1/system/settings/wrappers/:markid/versions/:versionId");
        ...
        app.get('/m-home/rest/v1/devices/me',
            ...
            function (request, response) {
                ...
                devicesMeV1Case.get(request, self.getUserId(request), self.getUserToken(request), null, request.query["type"], function (result) {
                    ...
                    self.sendMsg(response, result);
                    logger.debug(Date.now() - requestTime);
                });
            });
        ...
        app.post('/m-home/rest/v1/devices/me/actions',
            ...
            function (request, response) {
                var requestTime = Date.now();
                deviceActionsV1Case.post(request, self.getUserId(request), self.getUserToken(request), function (result) {
                    self.sendMsg(response, result);
                    logger.debug(Date.now() - requestTime);
                });
            });
        ...
        app.get('/m-home/rest/v1/devices',
            function (request, response) {
                devicesV1Case.get(request, function (result) {
                    self.sendMsg(response, result);
                });
            });
        ...
        //http
        var httpServer = http.createServer(app).listen(self.configurator.getConf("self.listen_port.http"), function () {
            var host = httpServer.address().address;
            var port = httpServer.address().port;
            logger.debug('Listening at https://' + host + ':' + port);
        });
        //https
        var privateKey = fs.readFileSync(path.join(__dirname, './private.pem'), 'utf-8');
        var certificate = fs.readFileSync(path.join(__dirname, './server.crt'), 'utf-8');
        var credentials = {key: privateKey, cert: certificate, passphrase: 'wzh83573100'};
        var httpsServer = https.createServer(credentials, app).listen(self.configurator.getConf("self.listen_port.https"), function () {

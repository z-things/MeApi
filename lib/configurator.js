...
function Configurator(host, port) {
  ...
  this.syncMeshbluServer = function (callback) {
    ...
    async.series({
        ...
        buffer_rate: function (innerCallback) {
          self.getZkNodeData("/system/meshblu_server/buffer_rate", function (error, path, data) {
            error && logger.error(error.errorId, error.errorMsg);
            error ? innerCallback(error) : innerCallback(null, parseInt(data));
          });
        }
  ...
  this.updateServices = function (zkPath, serviceName, childrens) {
    var self = this;
      ...
      if (error) {
        ...
        if (pathNodes.length > 2) {
          ...
          if(!util.isNullOrUndefined(service)){
            ...
            services.push({uuid: uuid, online: data});
          }
    ...
    var watcher = function (event) {
      logger.debug(event);
      if ("NODE_DELETED " !== event.name) {
        self.getZkNodeData(event.path, watcher, updater);
      }
    };
    _.forEach(childrens, function (children) {
      self.getZkNodeData(zkPath + "/" + children + "/online", watcher, updater);
    });
  };
        ...
        if (util.isArray(IDs)) {
          ...
          if (!util.isNullOrUndefined(services)) {
            ...
            _.forEach(IDs, function (ID) {
              ...
              service && tempServices.push(service);
            });
            config.services[mapItem.name] = tempServices;
            logger.debug(config);
            var configPath = path.join(__dirname, '../config.json');
            fs.writeFileSync(configPath, JSON.stringify(config, null, 2));
          }
    ...
    self.getZkNodeChildren(servicesPath, function (error, path, data) {
      if (error) {
        ...
        _.forEach(data, function (serviceName) {
          var serviceClusterPath = servicesPath + "/" + serviceName + "/cluster";
          self.serviceMap.push({name: serviceName, zkPath: serviceClusterPath});
